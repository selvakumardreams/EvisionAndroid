...
public class PersonalInformationName extends BaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		num1Button.setOnTouchListener(new OnTouchListener() {
			public boolean onTouch(View v, MotionEvent event) {
				Log.i("Motion Event", "Get Action" + event.getAction());
				...
				if (ACTION_DOWN == event.getAction()) {
					...
					if (isFgKeypress == true) {
						...
						if((reStr=="") || (reStr == null) || (reStr == " ")) {
							...
							Log.i("PersonalInformationName","PersonalInformationName"+reStr);
							speak(" f g pressed");
							launchApp.setFlags(Intent.FLAG_ACTIVITY_CLEAR_WHEN_TASK_RESET);
							launchApp.setClassName("com.evision.android", "com.evision.android.PersonalInformationContact"); 
							launchApp.putExtra("PersonalInformationName", reStr);
				...
				if ( ACTION_DOWN == event.getAction()) {
					...
					if(reStr!=null){
						if(reStr.length()!=0){
							...
							speak(ResourceUtil.getStringFromResource(PersonalInformationName.this, R.string.nochar_delete));
						}
					}
				}
				else if (ACTION_UP == event.getAction()) {
	...
	public void onInit(int status) {
		speak("Welcome to Personal profile entery");
		speak("enter your name and select F g to continue");
