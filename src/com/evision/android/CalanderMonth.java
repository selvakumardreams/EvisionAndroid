...
public class CalanderMonth extends BaseActivity {
					...
					if (starPressed) {
						if (reStr!=null) {
							if (reStr.length()!=0) {
								...
								speak(ResourceUtil.getStringFromResource(CalanderMonth.this,R.string.nonum_delete));
						}
					}
					lookuptable();
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (ACTION_DOWN == event.getAction()) {
					...
					if (isFgKeypress == true) {
						stop();
						if(reStr.length() == 0) {
							speak("please enter the month "+reStr);
						} else if(Integer.parseInt(reStr) >= 1 && Integer.parseInt(reStr) <=12) {
							speak("month entered is "+reStr);
							waitForSpeechFinished();
							Intent launchApp = new Intent();
							launchApp.setFlags(Intent.FLAG_ACTIVITY_CLEAR_WHEN_TASK_RESET);
							launchApp.putExtra("Caldate", Caldate);
							launchApp.putExtra("Calmonth", reStr);

							launchApp.setClassName("com.evision.android", "com.evision.android.CalanderYear"); 
							startActivity(launchApp); finish();

						} else {
							speak("please enter the month bewteen 0 and 12");
							reStr="";
						}

					} else if (isHgKeypress == true) {

						launchApp.setClassName("com.evision.android", "com.evision.android.CalanderDate"); 
	...
	public void onInit(int status) {
		speak(ResourceUtil.getStringFromResource(CalanderMonth.this, R.string.CalanderMonth_welcome));
		super.onInit(status);
	}
